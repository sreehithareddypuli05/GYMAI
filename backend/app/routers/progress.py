...
def get_progress(
    ...
    pose_sessions = (
        db.query(models.PoseSession)
        .filter(models.PoseSession.user_id == current_user.id)
        .order_by(models.PoseSession.created_at.asc())
        .all()
    )
    ...
    for week_start in week_starts:
        ...
        total_sets = sum(session.completed_sets or 0 for session in week_sessions)
        week_pose_scores = [
            pose.form_score for pose in pose_sessions
            if week_start <= utc_day(pose.created_at) < week_end
        ]
        ...
        consistency_by_week.append(
            schemas.ProgressPoint(label=label, value=round(consistency_percentage, 1))
        )
        sets_by_week.append(
            schemas.ProgressPoint(label=label, value=float(total_sets))
        )
        form_by_week.append(
            schemas.ProgressPoint(
                label=label,
                value=round(sum(week_pose_scores) / len(week_pose_scores), 1) if week_pose_scores else 0.0,
            )
        )
