    ...
    <div className="min-h-screen bg-charcoal text-ink flex overflow-hidden">
      <div className="relative hidden w-[48%] flex-col justify-between overflow-hidden border-r border-surface-border bg-[#080808] p-9 xl:p-12 lg:flex">
        ...
        <div className="relative z-10">
          <p className="label-eyebrow mb-4">{t('auth.eyebrow')}</p>
          <h2 className="max-w-md font-display text-4xl font-semibold leading-[.95] tracking-[-.05em] text-ink xl:text-5xl">{t('auth.tagline')}</h2>
          ...
          <p className="mt-2 max-w-md text-sm leading-6 text-ink-muted">{t('auth.description')}</p>
        </div>
      ...
      <div className="relative flex flex-1 flex-col items-center justify-center px-6 py-12 sm:px-10">
        <div className="absolute right-5 top-5 flex items-center gap-2"><LanguageSwitcher /><ThemeToggle /></div>
        <div className="w-full max-w-sm">
          ...
          <p className="label-eyebrow mb-2">{t(`authText.${eyebrow}`, { defaultValue: eyebrow })}</p><h1 className="mb-2 font-display text-3xl font-semibold tracking-tight text-ink">{t(`authText.${title}`, { defaultValue: title })}</h1><p className="mb-8 text-sm leading-6 text-ink-muted">{t(`authText.${subtitle}`, { defaultValue: subtitle })}</p>{children}
        </div>
      </div>
    </div>
  )
