    ...
    <section id="intelligence" className="ai-feature-section relative overflow-hidden border-t border-white/10 py-24 sm:py-32">
      ...
      <div className="container-shell relative z-10">
        <motion.div initial={{opacity:0,y:30}} whileInView={{opacity:1,y:0}} viewport={{once:true,margin:'-80px'}} transition={{duration:.7}} className="max-w-3xl">
          ...
          <h2 className="cinematic-heading mt-5 text-5xl sm:text-7xl">{copy.aiTitle}</h2>
          <p className="mt-6 max-w-2xl text-sm leading-7 text-ink-muted">{copy.aiBody}</p>
        </motion.div>
        ...
        <div className="mt-14 grid gap-8 lg:grid-cols-[1.02fr_.98fr] lg:items-center">
          ...
          <div className="space-y-2">
            {features.map((f, i) => {
              const [title, body] = copy.capabilities[i]
              return <motion.button key={title} onClick={() => setActive(i)} onMouseEnter={() => setActive(i)} whileHover={{x:6}} className={`feature-effect-card w-full border p-4 text-left transition-all duration-300 ${i===active?'feature-effect-card-active':'border-white/10 bg-[#0b0b0b] hover:border-white/25'}`}>
                <div className="flex items-center gap-4">
                  <div className="feature-thumb" aria-hidden="true"><img src={f.image} alt=""/><span className="feature-thumb-handle" /></div>
                  <div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-3"><h3 className="text-base font-semibold text-white">{title}</h3><f.icon size={17} className={i===active?'text-[#ff5a00]':'text-white/25'}/></div><p className="mt-1.5 text-xs leading-5 text-white/45">{body}</p></div>
